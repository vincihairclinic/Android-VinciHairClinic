        ...
        if (publication != null) {
            if (!publication.getTableOfContents().isEmpty()) {
                ...
                for (Link tocLink : publication.getTableOfContents()) {
                    TOCLinkWrapper tocLinkWrapper = new TOCLinkWrapper(tocLink, 0);
                    tocLinkWrappers.add(tocLinkWrapper);
                    for (Link temp : tocLink.getChildren()) {
                        TOCLinkWrapper tocLinkWrapper1 = new TOCLinkWrapper(temp, 1);
                        tocLinkWrappers.add(tocLinkWrapper1);
                    }
                }
