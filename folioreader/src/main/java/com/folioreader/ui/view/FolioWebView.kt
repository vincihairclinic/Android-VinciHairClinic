...
import kotlinx.android.synthetic.main.text_selection_colors.view.*
import kotlinx.android.synthetic.main.text_selection_exist.view.*
import kotlinx.android.synthetic.main.text_selection_exist.view.addNote
import kotlinx.android.synthetic.main.text_selection_main.view.*
...
//        }
    ...
    private var lastHighlightStyle: HighlightStyle = HighlightStyle.Yellow

    fun initViewTextSelection() {
        Log.v(LOG_TAG, "-> initViewTextSelection")
        viewTextSelection =
            LayoutInflater.from(context).inflate(R.layout.text_selection_main, null)?.apply {
                this.addNote?.setOnClickListener {
                    parentFragment.highlightWithNote(lastHighlightStyle, false)
                    dismissPopupWindow()
                }
                this.highlight?.setOnClickListener {
                    parentFragment.highlight(lastHighlightStyle, false)
                    dismissPopupWindow()
                }
            }
    ...
    fun setSelectionRect(left: Int, top: Int, right: Int, bottom: Int) {
        ...
        initViewTextSelectionForExistHightlight(currentSelectionRect)
        computeTextSelectionRect(currentSelectionRect)
        uiHandler.post { showTextSelectionPopup() }
    }

    private fun initViewTextSelectionForExistHightlight(currentSelectionRect: Rect) {
        Log.v(LOG_TAG, "-> initViewTextSelection")

        viewTextSelection =
            LayoutInflater.from(context).inflate(R.layout.text_selection_exist, null)?.apply {
                this.addNote?.setOnClickListener {
                    parentFragment.highlightWithNote(lastHighlightStyle, true)
                    dismissPopupWindow()
                }
                this.deleteHighlight?.setOnClickListener {
                    dismissPopupWindow()
                    loadUrl("javascript:clearSelection()")
                    loadUrl("javascript:deleteThisHighlight()")
                }
                this.showColors?.setOnClickListener {
                    viewTextSelection =
                        LayoutInflater.from(context).inflate(R.layout.text_selection_colors, null)
                            ?.apply {
                                this.yellowHighlight?.setOnClickListener {
                                    lastHighlightStyle = HighlightStyle.Yellow
                                    onHighlightColorItemsClicked(HighlightStyle.Yellow, true)
                                }
                                this.greenHighlight?.setOnClickListener {
                                    lastHighlightStyle = HighlightStyle.Green
                                    onHighlightColorItemsClicked(HighlightStyle.Green, true)
                                }
                                this.blueHighlight?.setOnClickListener {
                                    lastHighlightStyle = HighlightStyle.Blue
                                    onHighlightColorItemsClicked(HighlightStyle.Blue, true)
                                }
                                this.pinkHighlight?.setOnClickListener {
                                    lastHighlightStyle = HighlightStyle.Pink
                                    onHighlightColorItemsClicked(HighlightStyle.Pink, true)
                                }
                                this.underlineHighlight?.setOnClickListener {
                                    lastHighlightStyle = HighlightStyle.Underline
                                    onHighlightColorItemsClicked(HighlightStyle.Underline, true)
                                }
                            }
                    viewTextSelection?.measure(MeasureSpec.UNSPECIFIED, MeasureSpec.UNSPECIFIED)
                    computeTextSelectionRect(currentSelectionRect)
    ...
    @JavascriptInterface
    fun createSelectionRect(left: Int, top: Int, right: Int, bottom: Int) {
        val currentSelectionRect = Rect()
        currentSelectionRect.left = (left * density).toInt()
        currentSelectionRect.top = (top * density).toInt()
        currentSelectionRect.right = (right * density).toInt()
...
//        }
        ...
        viewTextSelection?.let { viewTextSelection ->
